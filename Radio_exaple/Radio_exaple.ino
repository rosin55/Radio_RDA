/// 
/// \На основе файла ScanRadio.ino из примеров библиотеки RDA5807M
...
void runSerialCommand(char cmd, int16_t value)
{
  ...
  if (cmd == '?') {
    ...
    Serial.println("x debug info from chip");
    Serial.println("f<val> set frequency = val");
  }
  ...
  // ----- control the frequency -----
    ...
    while (f <= fMax) {
      ...
      if ((ri.tuned) && (ri.rssi > 40) && (ri.snr > 10)) {
        radio.checkRDS();
...
void setup() {
  ...
  radio.setBandFrequency(RADIO_BAND_FM, 10470);
